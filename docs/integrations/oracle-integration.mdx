...
BEGIN
 ...
 FOR table IN (SELECT owner, table\_name FROM all\_tables WHERE owner='<schema>') LOOP

 EXECUTE IMMEDIATE 'grant select on '||table.owner||'.'||table.table\_name||' to secoda';
...
1. 1.  
In the Secoda App, select "New Integration" from <https://app.secoda.co/integrations>
