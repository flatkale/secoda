---
title: "Redash Integration"
description: "This page walks through the Secoda and Redash integration that Secoda supports"
---

## **Getting Started with Redash**

There are three steps to get started using Redash with Secoda:

1. 1.  
Retrieve your Redash Access Token

2. 2.  
Retrieve your Redash host

3. 3.  
Connect Redash to Secoda

#### Create an Access Token

To retreive your Redash access token for Secoda visit [Redash](https://app.redash.io) and log into your account. Use the following steps to generate an access token:

1. 1.  
Click on your avatar in the top right and select 'Edit profile' from the dropdown menu.

2. 2.  
Scroll to the section 'API Key'

3. 3.  
Copy the API Key and save it. This will be used to connect to Secoda.

#### Retrieve your host

Your **host** is the first part of the url in Redash up to the end of the site name. For example: `https://app.redash.io/secoda`

#### Connect Redash to Secoda

After retrieving your Redash access token and host , the next step is to connect to Secoda:

1. 1.  
In the Secoda App, select ‘Add Integration’ on the Integrations tab

2. 2.  
Search for and select ‘Redash’

3. 3.  
Enter your Redash access token and host you retrieved above

4. 4.  
Click 'Connect'