...
description: "This page walks through the Secoda and Connecting via SSH integration that Secoda supports"
---
...
Add the **public key** we provided you from the <https://app.secoda.co/tunnels/new> interface, _do not use the following_, it is just an example. On your **Bastion** host:
...
* Check that the public key **_we provide you_** on tunnel creation is in their `~/.ssh/authorized_keys` file.
...
ssh -L localhost:1111:<POSTGRES\_URL\_OR\_IP>:5432 -i <PRIVATE\_KEY\_NAME>.pem <BASTION\_USER>@<BASTION\_IP> psql -h localhost -P 1111 -U secodapostgres
